...
class ControllerExtensionPaymentOPAliPayHK extends Controller {
	...
	public function index() {
		$this->load->language('extension/payment/op_alipayHK');
 		...
 		if (isset($this->error['securecode'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
   			...
   			'href' => $this->url->link('extension/payment/op_alipayHK', 'token=' . $this->session->data['token'], 'SSL'),
   		);
				
		$data['action'] = $this->url->link('extension/payment/op_alipayHK', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['op_alipayHK_3d'])) {
			$data['op_alipayHK_3d'] = $this->request->post['op_alipayHK_3d'];
		} else {
			$data['op_alipayHK_3d'] = $this->config->get('op_alipayHK_3d');
		}
		
		if (isset($this->request->post['op_alipayHK_3d_terminal'])) {
			$data['op_alipayHK_3d_terminal'] = $this->request->post['op_alipayHK_3d_terminal'];
		} else {
			$data['op_alipayHK_3d_terminal'] = $this->config->get('op_alipayHK_3d_terminal');
		}
		
		if (isset($this->request->post['op_alipayHK_3d_securecode'])) {
			$data['op_alipayHK_3d_securecode'] = $this->request->post['op_alipayHK_3d_securecode'];
		} else {
			$data['op_alipayHK_3d_securecode'] = $this->config->get('op_alipayHK_3d_securecode');
		}
		...
		if (isset($this->request->post['op_alipayHK_currencies_value'])) {
			$data['op_alipayHK_currencies_value'] = $this->request->post['op_alipayHK_currencies_value'];
		} else {
			$data['op_alipayHK_currencies_value'] = $this->config->get('op_alipayHK_currencies_value');
		}
		...
		if (isset($this->request->post['op_alipayHK_country_array'])) {
			$data['op_alipayHK_country_array'] = $this->request->post['op_alipayHK_country_array'];
		} elseif ($this->config->has('op_alipayHK_country_array')) {
			$data['op_alipayHK_country_array'] = $this->config->get('op_alipayHK_country_array');
		} else {
			$data['op_alipayHK_country_array'] = array();
		}
		
		
		$data['callback'] = HTTP_CATALOG . 'index.php?route=extension/payment/op_alipayHK/callback';
		...
		if (isset($this->request->post['op_alipayHK_pay_mode'])) {
			$data['op_alipayHK_pay_mode'] = $this->request->post['op_alipayHK_pay_mode'];
		} else {
			$data['op_alipayHK_pay_mode'] = $this->config->get('op_alipayHK_pay_mode');
		}
		...
		if (isset($this->request->post['op_alipayHK_location'])) {
          $data['op_alipayHK_location'] = $this->request->post['op_alipayHK_location'];
		  } else {
			  $data['op_alipayHK_location'] = $this->config->get('op_alipayHK_location');
		  }

		  if (isset($this->request->post['op_alipayHK_locations'])) {
			  $data['op_alipayHK_locations'] = $this->request->post['op_alipayHK_locations'];
		  } else {
			  $data['op_alipayHK_locations'] = $this->config->get('op_alipayHK_locations');
		  }

		  if (isset($this->request->post['op_alipayHK_entity'])) {
			  $data['op_alipayHK_entity'] = $this->request->post['op_alipayHK_entity'];
		  } else {
			  $data['op_alipayHK_entity'] = $this->config->get('op_alipayHK_entity');
		  }

		  if (isset($this->request->post['op_alipayHK_entitys'])) {
			  $data['op_alipayHK_entitys'] = $this->request->post['op_alipayHK_entitys'];
		  } else {
			  $data['op_alipayHK_entitys'] = $this->config->get('op_alipayHK_entitys');
		  }

		//存入session
		  $this->session->data['op_alipayHK_location'] = $data['op_alipayHK_location'];
		  $this->session->data['op_alipayHK_locations'] = $data['op_alipayHK_locations'];
		  $this->session->data['op_alipayHK_entity'] = $data['op_alipayHK_entity'];
		  $this->session->data['op_alipayHK_entitys'] = $data['op_alipayHK_entitys'];
			...
			$this->response->setOutput($this->load->view('extension/payment/op_alipayHK.tpl', $data));
		}
		

	private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/payment/op_alipayHK')) {
